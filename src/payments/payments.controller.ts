...
export class PaymentsController {
  ...
  @Post('create-payment-selected')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Create payment URL for selected cart items' })
  async createPaymentFromSelectedCartItems(
    @Body()
    createPaymentDto: {
    ...
    this.logger.log(`Creating selected cart payment for user: ${userId}`);
    this.logger.debug(`Token payload: ${JSON.stringify(req.user)}`);

    // Set the correct userId in the cart object
    if (createPaymentDto.cartPaymentDto.cart) {
      createPaymentDto.cartPaymentDto.cart.userId = userId;
    }

    try {
      // Verify cart belongs to the authenticated user
      const cart = await this.cartService.findOne(
        createPaymentDto.cartPaymentDto.cart._id,
      );

      if (!cart) {
        this.logger.warn(
          `Cart not found: ${createPaymentDto.cartPaymentDto.cart._id}`,
        );
        throw new NotFoundException(
          `Cart with ID ${createPaymentDto.cartPaymentDto.cart._id} not found`,
        );
      }
      ...
      if (cart.userId && String(cart.userId) !== String(userId)) {
        this.logger.warn(
          `Unauthorized cart access attempt: ${createPaymentDto.cartPaymentDto.cart._id} by user: ${userId}, cart belongs to: ${cart.userId}`,
        );
        throw new UnauthorizedException('You do not have access to this cart');
      }

      this.logger.log(
        `Creating selected cart payment with ${createPaymentDto.selectedProductIds.length} selected items`,
      );

      const ipAddr = req.ip || req.socket.remoteAddress || '127.0.0.1';

      return this.paymentsService.createPaymentUrlFromCart(
        createPaymentDto.cartPaymentDto,
        ipAddr,
        userId,
        createPaymentDto.selectedProductIds,
      );
    } catch (error) {
      this.logger.error(
        `Error in selected cart payment: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  @Get('verify-selected')
  @ApiOperation({ summary: 'Verify payment return for selected cart items' })
  async verifySelectedPaymentReturn(@Query() query: any) {
    this.logger.log('Verifying selected payment return');
    return this.paymentsService.verifyReturnUrlWithSelectedItems(query);
  }
}
